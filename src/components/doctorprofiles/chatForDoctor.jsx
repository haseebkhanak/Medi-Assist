...
export default function ChatRoomDoctor() {
    ...
    const submitMessage = (event) => {
        ...
        if (message.trim()) {
            const newMessage = {
                fromUserId: doctorUniqueId,
                username: "You",
                message,
            };

            socket.emit('privateMessage', { toUserId: patientUniqueId, message });
            setStoreMessages((prevMessages) => {
                const updatedMessages = [...prevMessages, newMessage];
                localStorage.setItem("storeMessages", JSON.stringify(updatedMessages));
                return updatedMessages;
        ...
        <>
            <nav className="bg-pink-700 flex w-full fixed top-0 left-0 items-center shadow-2xl">
                <h2 className="text-xl text-white" style={{ position: "absolute", marginLeft: "120px" }}>
                    <i>Patient {patientName}</i>
                </h2>
                <button className="text-white text-lg" onClick={doctor_home_dashboard} style={{ marginLeft: "1000px" }}>
                    Home
                </button>
                <div>
                    <p className="text-white text-xl font-black ml-20">MEDI ASSIST</p>
                </div>
            </nav>
            ...
            <div className="sendchat" style={{ position: "absolute", overflowY: "auto", maxHeight: "400px" }}>
                ...
                {storeMessages
                    .filter((msg) => msg.fromUserId === patientUniqueId || msg.fromUserId === doctorUniqueId)
                    .map((msg, index) => (
                        <div
                            key={index}
                            className={msg.fromUserId === doctorUniqueId ? "message-sent" : "message-received"}
                ...
                <div className="flex">
                    ...
                    <div
                        style={{
                            ...
                            boxShadow: "5px 5px 20px rgb(107, 102, 102)",
                            borderRadius: "20px",
                        }}
                    >
                        <svg
                            ...
                            style={{ marginLeft: "16px", marginTop: "7px", cursor: "pointer" }}
                        >
                            <path d="M3.478 2.404a.75.75 0 0 0-.926.941l2.432 7.905H13.5a.75.75 0 0 1 0 1.5H4.984l-2.432 7.905a.75.75 0 0 0 .926.94 60.519 60.519 0 0 0 18.445-8.986.75.75 0 0 0 0-1.218A60.517 60.517 0 0 0 3.478 2.404Z" />
                        </svg>
                    </div>
