...
export default function GeneralProfiles(){
...
const destroysession= async()=>{
    ...
    const fetchGeneralProfiles= async()=>{
        try {
            const res= await fetch('http://localhost:2000/generalprofiles',{
                method:"POST",
                headers:{
                    "Content-Type":"application/json"
                }       
...
{message.length>0 && message.map((doctorProfile) =>
    ...
    <p className="text-center text-gray-600 text-3xl mt-40">
      Oops! No doctor is available for this specialization...
    </p>
  )}
